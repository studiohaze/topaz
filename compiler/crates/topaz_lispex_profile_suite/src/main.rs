...
const CORPUS_SCHEMA: &str = "topaz.psh-c3-profile-observation-corpus/v1";
...
fn parse_cases(bytes: &[u8]) -> Result<Vec<Case>, String> {
    ...
    for line in lines {
        ...
        if !matches!(
            fields[3],
            "complete" | "semantic-failure" | "prepare-refusal"
        ) {
        ...
        cases.push(Case {
            ...
            payload: decode_payload(fields[5])?,
        });
...
fn decode_payload(text: &str) -> Result<Vec<u8>, String> {
    if text == "-" {
        Ok(Vec::new())
    } else {
        decode_hex(text)
...
fn run(runtime: &Runtime, cases: &[Case]) -> Result<Vec<Observation>, String> {
    ...
    for case in cases {
        ...
        if case.category == "prepare-refusal" {
            if prepared.category != Category::RequestRefusal
                || prepared.code != case.code
                || !prepared.payload.is_empty()
            {
                return Err(format!(
                    "case-{}-prepare-mismatch-actual={}:{}:{}-expected={}:{}:{}",
                    case.id,
                    prepared.category.label(),
                    prepared.code,
                    sha256(&prepared.payload),
                    case.category,
                    case.code,
                    sha256(&case.payload),
                ));
            ...
            observations.push(Observation {
                ...
                code: prepared.code,
                payload_sha256: sha256(&prepared.payload),
            });
            continue;
        }
...
fn result_json(corpus_sha256: &str, observations: &[Observation]) -> String {
    ...
    format!(
        concat!(
            "{{\n",
            "  \"schema\": \"topaz.psh-c3-profile-observation-result/v1\",\n",
            "  \"status\": \"passed\",\n",
            "  \"candidateId\": \"topaz-lispex-private-embedding-candidate/c2r2\",\n",
...
fn main() {
    ...
    if let Err(error) = result {
        eprintln!("Lispex profile observation suite failed: {error}");
        std::process::exit(1);
    }
}
