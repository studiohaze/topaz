//! Checked-emission model owned by the backend-neutral Lowered IR.
//!
//! These shapes are populated once by `topaz_lower`; they are not parser AST
//! nodes and carry no source buffer. Exact lexical atoms live in `LoweredText`.
...
/// Exact lexical atoms needed by checked Rust emission.
///
/// The lowerer records only spans that the emission model can observe. It does
/// not retain a source file, permit arbitrary slicing, or expose host paths.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct LoweredText {
    identity: String,
    atoms: BTreeMap<(u32, u32, u32), String>,
    checked: bool,
    closure_bodies: BTreeMap<(u32, u32, u32), (u32, u32, u32)>,
    captures: BTreeMap<(u32, u32, u32), Vec<String>>,
}
...
impl LoweredText {
    pub fn with_identity(identity: impl Into<String>) -> Self {
        Self {
            ...
            atoms: BTreeMap::new(),
            checked: false,
            closure_bodies: BTreeMap::new(),
            captures: BTreeMap::new(),
        }
    }

    pub fn insert(&mut self, span: Span, value: impl Into<String>) {
        self.atoms
            .insert((span.file.0, span.lo, span.hi), value.into());
    }
    ...
    pub fn record_closure_body(&mut self, closure: Span, body: Span) {
        self.closure_bodies.insert(
            (closure.file.0, closure.lo, closure.hi),
            (body.file.0, body.lo, body.hi),
        );
    }

    pub fn insert_capture(&mut self, closure: Span, name: impl Into<String>) {
        let closure_key = (closure.file.0, closure.lo, closure.hi);
        let body = self
            .closure_bodies
            .get(&closure_key)
            .copied()
            .unwrap_or(closure_key);
        let captures = self.captures.entry(body).or_default();
        let name = name.into();
    ...
    pub fn captures_for_body(&self, body: Span) -> Option<&[String]> {
        let body = (body.file.0, body.lo, body.hi);
        self.captures.get(&body).map(Vec::as_slice).or_else(|| {
            (self.checked
                && self
                    .closure_bodies
                    .values()
...
pub fn text(value: &LoweredText, span: Span) -> Option<&str> {
    value.get(span)
}
...
pub struct Expr {
    pub kind: ExprKind,
    pub span: Span,
    /// Checker/lowerer-owned evaluation and binding decision for call and pipe
    /// expressions. Emitters consume this fact instead of re-deriving it.
    pub call: Option<crate::CallPlan>,
}
...
fn stmt_contains_placeholder(stmt: &Stmt) -> bool {
    match &stmt.kind {
        ...
        StmtKind::Defer(value) | StmtKind::Expr(value) => contains_placeholder(value),
...
pub fn boundary_guardable(ty: &Type, src: &LoweredText, type_params: &[Ident]) -> bool {
    fn name_at(src: &LoweredText, span: Span) -> &str {
        src.get(span)
            .expect("Lowered IR omitted a lexical atom required by a type guard")
    }
